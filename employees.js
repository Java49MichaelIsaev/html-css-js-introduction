...
function createRandomEmployees(nEmployees, idDigits, minSalary, maxSalary) {
    ...
    for (i = 0; i < nEmployees; i++) {
        emp = { name: `Name: ${randomName()} - ID : ${randomIdDigits(idDigits)}`, year: randomAge(), salary: randomSalary(minSalary, maxSalary) }
        result.push(emp);
    }
    return result;
}
...
// console.log(createRandomEmployees(5, 4, 1000, 2000));
// getEmployeeBySalary(result, 1000, 2000)
// getAverageAge(result)


console.log(createRandomEmployees(20, 3, 100, 1000));
